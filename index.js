...
app.post("/user/logout", (req, res) => {
  try {
    //  get tokens sent by the client
    const token = req.headers.authorization.split(" ")[1];
    if (!token) {
      return res.status(401).json({ message: "Unauthorized" });
    }
    // add tokens to blacklist or process as needed
    // this example only removes certain tokens, but security considerations should be taken into account
    // for example, it is recommended to use a database table to disable tokens
    // here we delete from local storage for simplicity
